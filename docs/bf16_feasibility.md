...
Project precision policy has changed: FP32 and BF16 are both production targets. The final production BF16 compute policy is defined in `docs/bf16_compute_strategy.md`.
...
- write experimental BF16-initializer model copies under `artifacts/experiments/bf16_storage_only`
...
Do not treat this storage-only conversion as production BF16. The tool refuses to write into `models/onnx/bf16`; use `src/export/export_all.py --precision bf16` for production BF16 compute artifacts.
...
```text
artifacts/experiments/bf16_storage_only/audio_vae_encoder/audio_vae_encoder.onnx
artifacts/experiments/bf16_storage_only/audio_vae_encoder/audio_vae_encoder.onnx.data
artifacts/experiments/bf16_storage_only/audio_vae_decoder/audio_vae_decoder.onnx
artifacts/experiments/bf16_storage_only/audio_vae_decoder/audio_vae_decoder.onnx.data
artifacts/experiments/bf16_storage_only/prefill/voxcpm2_prefill.onnx
artifacts/experiments/bf16_storage_only/prefill/voxcpm2_prefill.onnx.data
artifacts/experiments/bf16_storage_only/decode_step/voxcpm2_decode_step.onnx
artifacts/experiments/bf16_storage_only/decode_step/voxcpm2_decode_step.onnx.data
```

These files are ignored artifacts and must not be copied into `models/onnx/bf16`.
...
python -B src/cli/synthesize.py \
  ...
  --mode text_only \
  --audio-encoder-onnx artifacts/experiments/bf16_storage_only/audio_vae_encoder/audio_vae_encoder.onnx \
  --audio-decoder-onnx artifacts/experiments/bf16_storage_only/audio_vae_decoder/audio_vae_decoder.onnx \
  --prefill-onnx artifacts/experiments/bf16_storage_only/prefill/voxcpm2_prefill.onnx \
  --decode-step-onnx artifacts/experiments/bf16_storage_only/decode_step/voxcpm2_decode_step.onnx
```
...
Production BF16 compute now uses `src/export/export_all.py --precision bf16`, which changes wrapper/model compute dtype instead of adding initializer cast-back nodes. If ONNX Runtime CPU rejects a BF16 graph, or if parity exceeds tolerance, document the affected region as an FP32 island instead of using storage-only conversion as a substitute.

For this pass, ORT CPU coverage is sufficient for conservative BF16-initializer copies of all four modules. Production BF16 compute coverage must be proven by the checks in `docs/bf16_compute_strategy.md`.
