#!/usr/bin/env python3
"""Check and run the production VoxCPM2 decode-chunk module with ONNX Runtime CPU"""
...
def _make_inputs(args: argparse.Namespace) -> dict[str, np.ndarray]:
    ...
    return {
        ...
        "diffusion_noise": rng.standard_normal(
            (args.chunk_size, args.batch_size, args.feat_dim, args.patch_size),
            dtype=np.float32,
        ),
...
def run_decode_chunk(args: argparse.Namespace) -> None:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Path-check and run the production VoxCPM2DecodeChunk ONNX graph with ONNX Runtime CPU.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        epilog=(
            "Example: python -B src/runtime/run_decode_chunk_ort.py "
            "--onnx-path models/onnx/fp32/decode_chunk/voxcpm2_decode_chunk.onnx "
            "--chunk-size 4 --cache-seq 16 --max-cache-seq 64"
        ),
    )
...
def main() -> int:
    run_decode_chunk(_parser().parse_args())
    return 0
